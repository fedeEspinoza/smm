class Persona < ActiveRecord::Base
  ...
  validates :apellido, :format => {:with => /\A[a-z ñáéíóúA-ZÑÁÉÍÓÚ]+\z/, :message => "El campo Apellido sólo debe contener letras" }
  validates :nombre, :presence => { :message => "Debe completar el campo Nombre" }
  validates :nombre, :format => {:with => /\A[a-z ñáéíóúA-ZÑÁÉÍÓÚ]+\z/, :message => "El campo Nombre sólo debe contener letras" }
